...
class PositionMonitor:
    ...
    def __init__(self):
        ...
        self.config: dict = {}
        # Last known LTP per symbol — lets server.py's GET /api/positions report
        # live premium / unrealised P&L without making a synchronous broker call
        # on every dashboard poll. Updated on every WS tick and poll cycle.
    ...
    def start(self, config: dict) -> None:
        if self.running:
            # Guards against double-start spawning a second pair of monitor/WS
            # threads (e.g. if a future config-reload path ever calls start()
            # again — today only server.py's one-time FastAPI startup hook does).
            log.warning("Trade Manager start() called while already running — ignoring.")
            return
    ...
    def open_position(self, order_result: dict, req, config: dict, timeframe: str = None) -> None:
        """
        Called by app.py's TimeframeWorker right after a successful order is
        placed (BOT-Options fires orders autonomously from its signal-detection
        threads — there's no central "place order" endpoint to hang this off,
        unlike Bot-Stocks' manual-click flow. See app.py's _place_order()).

        Computes initial SL + target, persists to DB, and starts monitoring.

        Parameters
        ----------
        timeframe : the (symbol, timeframe) worker's timeframe string, passed
            explicitly since BOT-Options has no single global "scan_timeframe"
            config key the way Bot-Stocks does (it runs several timeframes at
            once across equity/trend/option workers).
        """
        tm_cfg  = config.get("trade_management", {})
        sl_pct  = float(tm_cfg.get("stop_loss_pct", 20.0))
        tgt_pct = float(tm_cfg.get("target_pct", 40.0))

        # Resolve entry price. Unlike Bot-Stocks (which only trusts req.price
        # for LIMIT orders), BOT-Options always resolves a live LTP or bar-close
        # price *before* calling place_order (see app.py's _check_and_alert),
        # regardless of order type — so req.price is a reliable estimate here
        # for MARKET orders too, and we don't need the order_result lookup that
        # OpenAlgo's placeorder() response doesn't actually populate.
        ...
        # Resolve options metadata (underlying/strike/expiry/lot size) so the
        # dashboard doesn't have to parse the symbol string itself.
        contract = get_instrument_master().lookup(req.symbol, req.exchange)
        ...
        pos_dict = {
            ...
            "underlying":          contract.underlying if contract.is_option else None,
            "strike":              contract.strike if contract.is_option else None,
            "option_type":         contract.option_type if contract.is_option else None,
            "expiry_date":         contract.expiry.isoformat() if contract.expiry else None,
            "lot_size":            contract.lot_size if contract.is_option else None,
        }
