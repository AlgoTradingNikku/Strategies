...
def calc_pnl_amount(pos: dict, price: float) -> float:
    """
    Return the rupee P&L for the position's *current* quantity at a given
    price (mark-to-market when called with LTP, realised when called with
    the exit price).

    `quantity` on a position is already the total contract units passed to
    OpenAlgo's placeorder() — i.e. lot_size x number_of_lots, not a lot
    count — so no further multiplication by lot size is needed here. See
    calc_lots() below for the human-readable "N lots" figure.
    """
    entry = float(pos["entry_price"])
    qty = float(pos.get("quantity", 0))
    if pos["direction"] == "BUY":
        return round((price - entry) * qty, 2)
    return round((entry - price) * qty, 2)


def calc_lots(pos: dict) -> Optional[float]:
    """
    Return quantity expressed in lots (quantity / lot_size), or None if the
    position isn't an option contract or lot_size couldn't be resolved.
    Rounded to 2dp to tolerate any partial-exit quantity that isn't an exact
    multiple of lot_size.
    """
    lot_size = pos.get("lot_size")
    qty = pos.get("quantity")
    if not lot_size or not qty:
        return None
    return round(float(qty) / float(lot_size), 2)
