...
class TradePersistence:
    ...
    def load_active_trades(self) -> List[Trade]:
        ...
        for row in cursor.fetchall():
            trade_dict = {
                ...
                "last_stage": row[15],
                "obs_candles": row[16],
                "obs_start_time": row[17],
                "idx_at_resolution": row[18],
                "expiry_params": json.loads(row[19]) if row[19] else {},
                "metadata": json.loads(row[20]) if row[20] else {},
                "trend_reversed": bool(row[21]),
                "manual_exit_pending": bool(row[22]),
            }
    ...
    def archive_trade(self, trade: Trade):
        ...
        """, (
            ...
            trade_dict["side"],
            trade_dict["entry_price"],
            trade_dict["current_price"],  # exit_price
            trade_dict["quantity"],
            trade_dict["entry_time"],
            trade_dict["exit_time"],
            trade_dict["exit_reason"],
