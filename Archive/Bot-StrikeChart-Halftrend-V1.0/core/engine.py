...
class TradingEngine:
            ...
            if symbol in self.trades and self.trades[symbol].state in [TradeState.ENTERING, TradeState.POSITION, TradeState.EXITING]:
                ...
                if trigger_active:
                    ...
                    if filters_pass:
                        ...
                        # Capture ML Snapshot for training before execution
                        snapshot_id = None
                        try:
                            use_ha_ltf = self.config.get("option", {}).get("ltf", {}).get("use_ha", False)
                            tech_res = self.indicators["option_tech"].calculate(df_opt, use_ha=use_ha_ltf)
                            snapshot_id = self.ml_filter.capture_snapshot(symbol, df_opt, tech_res.metadata)
                        except Exception as snapshot_err:
                            logger.error(f"Snapshot capture failed: {snapshot_err}")
    ...
    async def _check_entry_conditions(self, symbol: str, df_opt) -> tuple:
        ...
        # 9. ML FILTER VETO (Phase 2 - Predictive Suggestion)
        current_ml_id = None
        if not reasons: # Only predict if all other filters passed
            pass_ml, score, ml_reason = self.ml_filter.predict(symbol, df_opt, meta)
            if not pass_ml:
                reasons.append(f"ML REJECT: {ml_reason}")
            else:
                # If it passed and is collecting data, we still show the score
                if "Collecting" not in ml_reason:
                    print(f"       [ML] Confidence Score: {score*100:.1f}%")
    ...
    async def _execute_entry(self, symbol: str, side: str, price: float, ltf_signal, atr: float = 0.0, ml_snapshot_id: str = None):
            ...
            try:
                ...
                if order_id and order_id.success:
                    ...
                    trade = Trade(
                        ...
                        highest_price=price,
                        atr=atr,
                        metadata={"ml_snapshot_id": ml_snapshot_id} if ml_snapshot_id else {}
                    )
    ...
    async def _execute_exit(self, trade: Trade, reason: str):
        ...
        try:
            ...
            if order_id:
                ...
                # Update ML outcome for training (Phase 2)
                ml_id = trade.metadata.get("ml_snapshot_id")
                if ml_id:
                    self.ml_filter.update_outcome(ml_id, pnl, pnl_pct)
